...
print('============================================================')
print(f'The name of this model is: {model_name}')
print('============================================================')
...
shutil.copy(config['config_dir'], model_dir + 'config.yaml')
...
for validation_fold in range(config_train['num_folds']):
    ...
    #If specifying labels to keep or relabel output dim is explicitly specified
    if config_dsop['selected_labels'] or config_dsop['relabel']:
        output_dim = len(config_dsop['selected_labels']) if not config_dsop['relabel'] else len(config_dsop['mapped_labels'])
    #Else output dim equals max label (plus 1 for 0)
    else:
        output_dim = max([torch.max(i[1]) for i in itertools.islice(iter(train_dataset), 10)]) + 1
    ...
    utils.create_confusion_matrix(pred_labels[best_model_index],true_labels[best_model_index],confusion_matrix_file_path)
...
#Print out helpful instructions and best model results
with open(model_dir + '/results.csv') as results:
...
print(f'These model results can be found in {model_dir}. The best performing fold was fold {best_fold} with validation accuracy (%) = {highest_val_acc}')

#Delete dataset to save drive space
os.remove(h5_path)
